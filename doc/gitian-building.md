...
*Setup instructions for a gitian build of ORO Core using a VM or physical system.*
...
to the ORO GitHub release page.
...
- [Building ORO Core](#building-oro-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/oro-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/oro-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/oro-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/oro-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/oro-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/oro-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/oro-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@oro.cm) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/oro-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/oro-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/oro-osx-*-build.assert
```
