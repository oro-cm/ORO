...
//    unsigned int nBlockMaxSize = GetArg("-blockmaxsize", DEFAULT_BLOCK_MAX_SIZE);
...

    {
        ...
        UpdateTime(pblock, pindexPrev); // ORO: do this before getting reward from ORO

        // Compute final coinbase transaction.
        txNew.vout[0].nValue = pOroSystem->GetBlockRewardForNewTip(pblock->nTime) + nFees;
    ...
    try {
        while (true) {
            ...
            while (true) {
                ...
                break;  // ORO: we can't update nTime without breaking the block, so just rebuild it
