...
CzOROTracker::CzOROTracker(std::string strWalletFile)
{
...
CzOROTracker::~CzOROTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzOROTracker::Init()
{
...
bool CzOROTracker::Archive(CMintMeta& meta)
{
...
bool CzOROTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzOROTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzOROTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzOROTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzOROTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzOROTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzOROTracker::GetSerialHashes()
{
...
CAmount CzOROTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzOROTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzOROTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzOROTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzOROTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzOROTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzOROTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzOROTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzOROTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzOROTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzOROTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzOROWallet* zOROWallet)
{
    bool iszOROWalletInitialized = (NULL != zOROWallet);
    CMintMeta meta;
    ...
    if (! iszOROWalletInitialized)
        zOROWallet = new CzOROWallet(strWalletFile);
    meta.isSeedCorrect = zOROWallet->CheckSeed(dMint);
    if (! iszOROWalletInitialized)
        delete zOROWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzOROTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzOROTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzOROTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzOROTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzOROTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzOROTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzOROWallet* zOROWallet = new CzOROWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zOROWallet);
        }
        delete zOROWallet;
        LogPrint("zero", "%s: added %d dzoro from DB\n", __func__, listDeterministicDB.size());
    }
