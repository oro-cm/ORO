...
CzOROWallet::CzOROWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zoro seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZOROSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZOROSeed_deprecated()) {
                    LogPrintf("%s: Updated zORO seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zoro seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zoro wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzOROWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZOROCount(nCountLastUsed);
    else if (!walletdb.ReadZOROCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzOROWallet::Lock()
{
    seedMaster = 0;
}

void CzOROWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzOROWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZORO(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzOROWallet::LoadMintPoolFromDB()
{
...
void CzOROWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzOROWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzOROWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zoroTracker->HasPubcoinHash(pMint.first)) {
...
bool CzOROWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZORO(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zoroTracker which also adds to database
    pwalletMain->zoroTracker->Add(dMint, true);
...
void CzOROWallet::SeedToZORO(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzOROWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzOROWallet::GenerateDeterministicZORO(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzOROWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzOROWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzOROWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
