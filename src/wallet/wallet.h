...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zorospends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 200,000 ORO at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zORO send
enum ZerocoinSpendStatus {
    ZORO_SPEND_OKAY = 0,                            // No error
    ZORO_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZORO_WALLET_LOCKED = 2,                         // Wallet was locked
    ZORO_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZORO_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZORO_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZORO_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZORO_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZORO_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZORO_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZORO_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZORO_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZORO_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZORO_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZORO_SPENT_USED_ZORO = 14,                      // Coin has already been spend
    ZORO_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZORO_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZOroBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZOROOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    std::string GetUniqueWalletBackupName(bool fzoroAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzOROTracker> zoroTracker;
    ...
    void setZWallet(CzOROWallet* zwallet);
    CzOROWallet* getZWallet();
    bool isZeromintEnabled();
    void setZOroAutoBackups(bool fEnabled);
    ...
    /// Get 200,000 ORO output and keys which can be used for the Masternode
    ...
    CAmount GetOro(const CTxOut& txout, const isminefilter& filter) const;
    ...
    CAmount GetOro(const CTransaction& tx, const isminefilter& filter, const bool fUnspent = false) const;
    ...
    /** zORO reset */
    boost::signals2::signal<void()> NotifyzOROReset;
...
public:
    ...
    mutable bool fOroCached;
    mutable bool fImmatureOroCached;
    mutable bool fAvailableOroCached;
    mutable bool fAnonymizableOroCached;
    mutable bool fAnonymizedOroCached;
    mutable bool fDenomUnconfOroCached;
    mutable bool fDenomConfOroCached;
    mutable bool fWatchDebitCached;
    mutable bool fWatchOroCached;
    mutable bool fImmatureWatchOroCached;
    mutable bool fAvailableWatchOroCached;
    mutable bool fChangeCached;
    mutable bool fColdDebitCached;
    mutable bool fColdOroCached;
    mutable bool fDelegatedDebitCached;
    mutable bool fDelegatedOroCached;
    mutable bool fStakeDelegationVoided;
    mutable CAmount nDebitCached;
    mutable CAmount nOroCached;
    mutable CAmount nImmatureOroCached;
    mutable CAmount nAvailableOroCached;
    mutable CAmount nAnonymizableOroCached;
    mutable CAmount nAnonymizedOroCached;
    mutable CAmount nDenomUnconfOroCached;
    mutable CAmount nDenomConfOroCached;
    mutable CAmount nWatchDebitCached;
    mutable CAmount nWatchOroCached;
    mutable CAmount nImmatureWatchOroCached;
    mutable CAmount nAvailableWatchOroCached;
    mutable CAmount nChangeCached;
    mutable CAmount nColdDebitCached;
    mutable CAmount nColdOroCached;
    mutable CAmount nDelegatedDebitCached;
    mutable CAmount nDelegatedOroCached;
    ...
    CAmount GetOro(const isminefilter& filter) const;
    CAmount GetUnspentOro(const isminefilter& filter) const;
    CAmount GetImmatureOro(bool fUseCache = true, const isminefilter& filter = ISMINE_SPENDABLE_ALL) const;
    CAmount GetAvailableOro(bool fUseCache = true) const;
    // Return sum of unlocked coins
    CAmount GetUnlockedOro() const;
    // Return sum of unlocked coins
    CAmount GetLockedOro() const;
    CAmount GetImmatureWatchOnlyOro(const bool& fUseCache = true) const;
    CAmount GetAvailableWatchOnlyOro(const bool& fUseCache = true) const;
    CAmount GetLockedWatchOnlyOro() const;
    CAmount GetChange() const;

    // Cold staking contracts oro/debit
    CAmount GetColdStakingOro(bool fUseCache = true) const;
    CAmount GetColdStakingDebit(bool fUseCache = true) const;
    CAmount GetStakeDelegationOro(bool fUseCache = true) const;
    CAmount GetStakeDelegationDebit(bool fUseCache = true) const;

    // Helper method to update the amount and cacheFlag.
    CAmount UpdateAmount(CAmount& amountToUpdate, bool& cacheFlagToUpdate, bool fUseCache, isminetype mimeType, bool fOro = true) const;
