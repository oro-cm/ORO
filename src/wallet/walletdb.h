...
public:
    ...
    CAmount GetAccountOroDebit(const std::string& strAccount);
    void ListAccountOroDebit(const std::string& strAccount, std::list<CAccountingEntry>& acentries);
    ...
    bool WriteZOROSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZOROSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZOROSeed_deprecated(uint256& seed);
    bool EraseZOROSeed();
    bool EraseZOROSeed_deprecated();

    bool WriteZOROCount(const uint32_t& nCount);
    bool ReadZOROCount(uint32_t& nCount);
