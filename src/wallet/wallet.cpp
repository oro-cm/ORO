...
#include "zoro/zorowallet.h"
#include "zoro/zorotracker.h"
#include "zoro/deterministicmint.h"
#include <assert.h>
...
/**
 * Fees smaller than this (in uoro) are considered zero fee (for transaction creation)
...
bool CWallet::AddToWallet(const CWalletTx& wtxIn, bool fFromLoadWallet, CWalletDB* pwalletdb)
{
    ...
    if (fFromLoadWallet) {
        ...
        // Break debit/oro balance caches:
        wtx.MarkDirty();
...
{
    return zoroTracker->HasSerial(bnSerial);
}
...
CAmount CWalletTx::GetUnspentOro(const isminefilter& filter) const
{
    ...
    CAmount oro = 0;
    if (filter & ISMINE_SPENDABLE) {
        oro += pwallet->GetOro(*this, ISMINE_SPENDABLE, true);
    }
    if (filter & ISMINE_WATCH_ONLY) {
        oro += pwallet->GetOro(*this, ISMINE_WATCH_ONLY, true);
    }
    if (filter & ISMINE_COLD) {
        oro += pwallet->GetOro(*this, ISMINE_COLD, true);
    }
    if (filter & ISMINE_SPENDABLE_DELEGATED) {
        oro += pwallet->GetOro(*this, ISMINE_SPENDABLE_DELEGATED, true);
    }
    return oro;
}

CAmount CWalletTx::GetOro(const isminefilter& filter) const
{
    CAmount oro = 0;
    if (filter & ISMINE_SPENDABLE) {
        ...
        if (fOroCached)
            ...
            nOroCached = pwallet->GetOro(*this, ISMINE_SPENDABLE, false);
            fOroCached = true;
            oro += nOroCached;
        }
    }
    if (filter & ISMINE_WATCH_ONLY) {
        if (fWatchOroCached)
            ...
            nWatchOroCached = pwallet->GetOro(*this, ISMINE_WATCH_ONLY, false);
            fWatchOroCached = true;
            oro += nWatchOroCached;
        }
    }
    if (filter & ISMINE_COLD) {
        if (fColdOroCached)
            ...
            nColdOroCached = pwallet->GetOro(*this, ISMINE_COLD, false);
            fColdOroCached = true;
            oro += nColdOroCached;
        }
    }
    if (filter & ISMINE_SPENDABLE_DELEGATED) {
        if (fDelegatedOroCached)
            oro += nDelegatedOroCached;
        else {
            nDelegatedOroCached = pwallet->GetOro(*this, ISMINE_SPENDABLE_DELEGATED, false);
            fDelegatedOroCached = true;
            oro += nDelegatedOroCached;
        }
    }
    return oro;
...
CAmount CWalletTx::GetImmatureOro(bool fUseCache, const isminefilter& filter) const
{
    LOCK(cs_main);
    if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0 && IsInMainChain()) {
        if (fUseCache && fImmatureOroCached && filter == ISMINE_SPENDABLE_ALL)
            return nImmatureOroCached;
        nImmatureOroCached = pwallet->GetOro(*this, filter);
        fImmatureOroCached = true;
        return nImmatureOroCached;
    }
...
CAmount CWalletTx::GetAvailableOro(bool fUseCache) const
{
    return GetUnspentOro(ISMINE_SPENDABLE_ALL);
}

CAmount CWalletTx::GetColdStakingOro(bool fUseCache) const
{
    return GetUnspentOro(ISMINE_COLD);
}

CAmount CWalletTx::GetStakeDelegationOro(bool fUseCache) const
{
    return GetUnspentOro(ISMINE_SPENDABLE_DELEGATED);
}

CAmount CWalletTx::UpdateAmount(CAmount& amountToUpdate, bool& cacheFlagToUpdate, bool fUseCache, isminetype mimeType, bool fOro) const
{
    if (pwallet == 0)
        return 0;
    ...
    amountToUpdate = (fOro) ? GetOro(mimeType) : GetDebit(mimeType);
    cacheFlagToUpdate = true;
    return amountToUpdate;
}

// Return sum of unlocked coins
CAmount CWalletTx::GetUnlockedOro() const
{
    if (pwallet == 0)
        return 0;
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        nOro += pwallet->GetOro(txout, ISMINE_SPENDABLE);
        if (!MoneyRange(nOro))
            throw std::runtime_error("CWalletTx::GetUnlockedOro() : value out of range");
    }
...
CAmount CWalletTx::GetLockedOro() const
{
    if (pwallet == 0)
        return 0;
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        // Add delegated coins
        nOro += pwallet->GetOro(txout, ISMINE_SPENDABLE_DELEGATED);
        ...
        if (!MoneyRange(nOro))
            throw std::runtime_error("CWalletTx::GetLockedOro() : value out of range");
    }

    return nOro;
}

CAmount CWalletTx::GetImmatureWatchOnlyOro(const bool& fUseCache) const
{
    LOCK(cs_main);
    if (IsCoinBase() && GetBlocksToMaturity() > 0 && IsInMainChain()) {
        if (fUseCache && fImmatureWatchOroCached)
            return nImmatureWatchOroCached;
        nImmatureWatchOroCached = pwallet->GetOro(*this, ISMINE_WATCH_ONLY);
        fImmatureWatchOroCached = true;
        return nImmatureWatchOroCached;
    }
...
CAmount CWalletTx::GetAvailableWatchOnlyOro(const bool& fUseCache) const
{
    if (pwallet == 0)
        return 0;
    ...
    if (fUseCache && fAvailableWatchOroCached)
        return nAvailableWatchOroCached;

    CAmount nOro = 0;
    for (unsigned int i = 0; i < vout.size(); i++) {
        if (!pwallet->IsSpent(GetHash(), i)) {
            const CTxOut& txout = vout[i];
            nOro += pwallet->GetOro(txout, ISMINE_WATCH_ONLY);
            if (!MoneyRange(nOro))
                throw std::runtime_error("CWalletTx::GetAvailableOro() : value out of range");
        }
    }

    nAvailableWatchOroCached = nOro;
    fAvailableWatchOroCached = true;
    return nOro;
}

CAmount CWalletTx::GetLockedWatchOnlyOro() const
{
    if (pwallet == 0)
        return 0;
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate, bool fromStartup)
{
    ...
    bool fCheckZORO = GetBoolArg("-zapwallettxes", false);
    if (fCheckZORO)
        zoroTracker->Init();
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            //If this is a zapwallettx, need to readd zoro
            if (fCheckZORO && pindex->nHeight >= Params().Zerocoin_StartHeight()) {
...
CAmount CWallet::GetColdStakingBalance() const
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
        if (pcoin.HasP2CSOutputs() && pcoin.IsTrusted())
            nTotal += pcoin.GetColdStakingOro();
    });
}
...
CAmount CWallet::GetDelegatedBalance() const
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
            if (pcoin.HasP2CSOutputs() && pcoin.IsTrusted())
                nTotal += pcoin.GetStakeDelegationOro();
    });
}
...
{
    if (fMatureOnly) {
        // This code is not removed just for when we back to use zORO in the future, for now it's useless,
        ...
        CAmount nBalance = 0;
        std::vector<CMintMeta> vMints = zoroTracker->GetMints(true);
    ...
    return zoroTracker->GetBalance(false, false);
}
...
{
    return zoroTracker->GetUnconfirmedBalance();
}
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        std::set<CMintMeta> setMints = zoroTracker->ListMints(true, true, true);
...
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
            nTotal += pcoin.GetImmatureOro(false);
    });
}
...
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
            nTotal += pcoin.GetImmatureOro(false, ISMINE_COLD);
    });
}
...
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
            nTotal += pcoin.GetImmatureOro(false, ISMINE_SPENDABLE_DELEGATED);
    });
}
...
CAmount CWallet::GetWatchOnlyBalance() const
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
            if (pcoin.IsTrusted())
                nTotal += pcoin.GetAvailableWatchOnlyOro();
    });
}
...
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
            nTotal += pcoin.GetImmatureWatchOnlyOro();
    });
}
...
CAmount CWallet::GetLockedWatchOnlyBalance() const
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
            if (pcoin.IsTrusted() && pcoin.GetDepthInMainChain() > 0)
                nTotal += pcoin.GetLockedWatchOnlyOro();
    });
}
        ...
        int blockHeight, bool fPrecompute)
{
    ...
    if (GetBoolArg("-orostake", true) && !fPrecompute) {
        for (const COutput &out : vCoins) {
            ...
            std::unique_ptr<COroStake> input(new COroStake());
            input->SetInput((CTransaction) *out.tx, out.i);
            listInputs.emplace_back(std::move(input));
        }
    }

    /* Disable zORO Staking
    //zORO
    if ((GetBoolArg("-zorostake", true) || fPrecompute) && chainActive.Height() > Params().Zerocoin_Block_V2_Start() && !sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        //Only update zORO set once per update interval
        bool fUpdate = false;
        ...
        std::set<CMintMeta> setMints = zoroTracker->ListMints(true, true, fUpdate);
...
bool CWallet::MintableCoins()
{
    ...
    CAmount nZoroBalance = GetZerocoinBalance(false);
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 ORO collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    std::vector<std::pair<CScript, CAmount> > vecSend;
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 ORO collateral
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX, false, fIncludeDelegated)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 200,000 ORO.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 200,000 ORO.");
                    } else {
                ...
                for (PAIRTYPE(const CWalletTx*, unsigned int) pcoin : setCoins) {
                    ...
                    CAmount nOro = pcoin.first->vout[pcoin.second].nValue;
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-oro-address
                    CScript scriptChange;
        ...
        )
{
    ...
    for (std::unique_ptr<CStakeInput>& stakeInput : listInputs) {
        ...
        if (Stake(pindexPrev, stakeInput.get(), nBits, nTxNewTime, hashProofOfStake)) {
            ...
            if (!stakeInput->CreateTxOuts(this, vout, nOro)) {
            ...
            CAmount nMinFee = 0;
            if (!stakeInput->IsZORO()) {
                // Set output amount
                int outputs = txNew.vout.size() - 1;
                CAmount nRemaining = nOro - nMinFee;
                if (outputs > 1) {
            ...
            //Masternode payment
            FillBlockPayee(txNew, nMinFee, true, stakeInput->IsZORO());

            {
                TRY_LOCK(zoroTracker->cs_spendcache, fLocked);
                if (!fLocked)
                    continue;
            ...
            //Mark mints as spent
            if (stakeInput->IsZORO()) {
                CZOroStake* z = (CZOroStake*)stakeInput.get();
...
void CWallet::CreateAutoMintTransaction(const CAmount& nMintAmount, CCoinControl* coinControl)
{
    if (nMintAmount > 0){
        ...
        LogPrintf("CWallet::AutoZeromint() @ block %ld: successfully minted %ld zORO. Current percentage of zORO: %lf%%\n",
...
void CWallet::AutoZeromint()
{
    ...
    // zORO are integers > 0, so we can't mint 10% of 9 ORO
    if (nBalance < 10){
        LogPrint("zero", "CWallet::AutoZeromint(): available balance (%ld) too small for minting zORO\n", nBalance);
        return;
    }

    // Percentage of zORO we already have
    ...
    if(dPercentage >= nZeromintPercentage){
        LogPrint("zero", "CWallet::AutoZeromint() @block %ld: percentage of existing zORO (%lf%%) already >= configured percentage (%d%%). No minting needed...\n",
                  chainActive.Tip()->nHeight, dPercentage, nZeromintPercentage);
        return;
    }

    // zORO amount needed for the target percentage
    nToMintAmount = ((nZerocoinBalance + nBalance) * nZeromintPercentage / 100);

    // zORO amount missing from target (must be minted)
    nToMintAmount = (nToMintAmount - nZerocoinBalance) / COIN;

    // Use the biggest denomination smaller than the needed zORO We'll only mint exact denomination to make minting faster.
    ...
    // If a preferred denomination is used (means nPreferredDenom != 0) do nothing until we have enough ORO to mint this denomination
...
bool CWallet::MultiSend()
{
    ...
    for (const COutput& out : vCoins) {
        ...
        for (unsigned int i = 0; i < vMultiSend.size(); i++) {
            ...
            nAmount = ((out.tx->GetOro(filter) - out.tx->GetDebit(filter)) * vMultiSend[i].second) / 100;
...
bool CWallet::CreateZOROOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint)
{
    ...
    zwalletMain->GenerateDeterministicZORO(denomination, coin, dMint);
...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, std::vector<CDeterministicMint>& vDMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    ...
    while (true) {
        ...
        if (!CreateZOROOutPut(denomination, outMint, dMint)) {
            strFailReason = strprintf("%s: failed to create new zoro output", __func__);
    ...
    //also assume that a zerocoinspend that is minting the change will not have any change that goes to Oro
    ...
    // Sign if these are oro outputs - NOTE that zORO outputs are signed later in SoK
    if (!isZCSpendChange) {
        int nIn = 0;
        for (const std::pair<const CWalletTx*, unsigned int>& coin : setCoins) {
            if (!SignSignature(*this, *coin.first, txNew, nIn++)) {
...
bool CWallet::CheckCoinSpend(libzerocoin::CoinSpend& spend, libzerocoin::Accumulator& accumulator, CZerocoinSpendReceipt& receipt)
{
    if (!spend.Verify(accumulator)) {
        receipt.SetStatus(_("The transaction did not verify"), ZORO_BAD_SERIALIZATION);
    ...
    if (Params().NetworkID() != CBaseChainParams::REGTEST && IsSerialKnown(spend.getCoinSerialNumber())) {
        //Tried to spend an already spent zORO
        receipt.SetStatus(_("The coin spend has been used"), ZORO_SPENT_USED_ZORO);
        uint256 hashSerial = GetSerialHash(spend.getCoinSerialNumber());
        if(!zoroTracker->HasSerialHash(hashSerial))
            return error("%s: serialhash %s not found in tracker", __func__, hashSerial.GetHex());

        CMintMeta meta = zoroTracker->Get(hashSerial);
        meta.isUsed = true;
        if (!zoroTracker->UpdateState(meta))
                         ...
                         CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    receipt.SetStatus(_("Transaction Mint Started"), ZORO_TXMINT_GENERAL);
    ...
    while (nLockAttempts < 100) {
        TRY_LOCK(zoroTracker->cs_spendcache, lockSpendcache);
        if (!lockSpendcache) {
        ...
        for (auto &it : mapMintsSelected) {
            ...
            CMintMeta meta = zoroTracker->Get(GetSerialHash(mint.GetSerialNumber()));
            CoinWitnessData *coinWitness = zoroTracker->GetSpendCache(meta.hashStake);
            ...
            if (!GenerateAccumulatorWitness(coinWitness, mapAccumulators, pindexCheckpoint)) {
                receipt.SetStatus(_("Couldn't generate the accumulator witness"),
                                  ZORO_FAILED_ACCUMULATOR_INITIALIZATION);
            ...
            if (nVersion >= libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (!mint.GetKeyPair(key))
                    return error("%s: failed to set zORO privkey mint version=%d", __func__, nVersion);
            ...
            try {
                ...
                if (!CheckCoinSpend(spend, accumulator, receipt)) {
                    receipt.SetStatus(_("CoinSpend: failed check"), ZORO_SPEND_ERROR);
                    return error("%s : %s", __func__, receipt.GetStatusMessage());
                }
                ...
                receipt.SetStatus(_("CoinSpend: Accumulator witness does not verify"), ZORO_INVALID_WITNESS);
                return error("%s : %s", __func__, receipt.GetStatusMessage());
            }
        }
        break;
    ...
    if (nLockAttempts == 100) {
        ...
        receipt.SetStatus(_("could not get lock on cs_spendcache"), ZORO_TXMINT_GENERAL);
        return false;
    }
    ...
    receipt.SetStatus(_("Spend Valid"), ZORO_SPEND_OKAY); // Everything okay
    ...
    while (nLockAttempts < 100) {
        ...
        for (auto &it : mapMintsSelected) {
            ...
            if (!GetTransaction(mint.GetTxHash(), txMint, hashBlock)) {
                receipt.SetStatus(strprintf(_("Unable to find transaction containing mint %s"), mint.GetTxHash().GetHex()), ZORO_TXMINT_GENERAL);
                return false;
            } else if (mapBlockIndex.count(hashBlock) < 1) {
                // check that this mint made it into the blockchain
                receipt.SetStatus(_("Mint did not make it into blockchain"), ZORO_TXMINT_GENERAL);
                return false;
            }
            ...
            if (outputIndex == -1) {
                receipt.SetStatus(_("Pubcoin not found in mint tx"), ZORO_TXMINT_GENERAL);
                return false;
            }

            mint.SetOutputIndex(outputIndex);
            CTxIn in;
            if(!ZOROModule::createInput(in, mint, hashTxOut, spendVersion)) {
                receipt.SetStatus(_("Cannot create public spend input"), ZORO_TXMINT_GENERAL);
                return false;
            }
            vin.emplace_back(in);
        ...
        bool isPublicSpend)
{
    ...
    int nStatus = ZORO_TRX_FUNDS_PROBLEMS;
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the smallest available denomination (= 1) of zORO"), nStatus);
        return false;
    }

    // Create transaction
    nStatus = ZORO_TRX_CREATE;
    ...
    const int nMaxSpends = Params().Zerocoin_MaxPublicSpendsPerTransaction(); // Maximum possible spends for one zORO public spend transaction
    std::vector<CMintMeta> vMintsToFetch;
    if (vSelectedMints.empty()) {
        //  All of the zORO used in the public coin spend are mature by default (everything is public now.. no need to wait for any accumulation)
        setMints = zoroTracker->ListMints(true, false, true, true); // need to find mints to spend
        if(setMints.empty()) {
        ...
        // Select the zORO mints to use in this spend
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            for (std::pair<CBitcoinAddress*,CAmount> pair : addressesTo){
                ...
                //add output to oro address to the transaction (the actual primary spend taking place)
                // TODO: check value?
            ...
            if (nBytes >= MAX_ZEROCOIN_TX_SIZE) {
                receipt.SetStatus(_("In rare cases, a spend with 7 coins exceeds our maximum allowable transaction size, please retry spend using 6 or less coins"), ZORO_TX_TOO_LARGE);
                return false;
            }
    ...
    receipt.SetStatus(_("Transaction Created"), ZORO_SPEND_OKAY); // Everything okay
...
std::string CWallet::ResetMintZerocoin()
{
    ...
    std::set<CMintMeta> setMints = zoroTracker->ListMints(false, false, true);
...
void CWallet::ReconsiderZerocoins(std::list<CZerocoinMint>& listMintsRestored, std::list<CDeterministicMint>& listDMintsRestored)
{
    ...
    for (CZerocoinMint mint : listMints) {
        ...
        if (!zoroTracker->UnArchive(hashPubcoin, false)) {
            LogPrintf("%s : failed to unarchive mint %s\n", __func__, mint.GetValue().GetHex());
        } else {
            zoroTracker->UpdateZerocoinMint(mint);
        }
    ...
    for (CDeterministicMint dMint : listDMints) {
        ...
        if (!zoroTracker->UnArchive(dMint.GetPubcoinHash(), true)) {
            LogPrintf("%s : failed to unarchive deterministic mint %s\n", __func__, dMint.GetPubcoinHash().GetHex());
        } else {
            zoroTracker->Add(dMint, true);
        }
        listDMintsRestored.emplace_back(dMint);
    }
}

std::string CWallet::GetUniqueWalletBackupName(bool fzoroAuto) const
{
    ...
    return strprintf("wallet%s.dat%s", fzoroAuto ? "-autozorobackup" : "", DateTimeStrFormat(".%Y-%m-%d-%H-%M", GetTime()));
}

void CWallet::ZOroBackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        strNewBackupName = strprintf("wallet-autozorobackup-%d.dat", i);
        ...
        if (boost::filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                boost::filesystem::path nextBackupPath = backupDir / strprintf("wallet-autozorobackup-%d.dat", i + 1);
            ...
            strNewBackupName = strprintf("wallet-autozorobackup-%d.dat", 0);
            backupPath = backupDir / strNewBackupName;
            break;
        }
    ...
    if(!GetArg("-zorobackuppath", "").empty()) {
        boost::filesystem::path customPath(GetArg("-zorobackuppath", ""));
...
)
{
    ...
    int nStatus = ZORO_SPEND_ERROR;

    if (IsLocked()) {
        receipt.SetStatus("Error: Wallet locked, unable to create transaction!", ZORO_WALLET_LOCKED);
        return false;
    }
    ...
    if (!CommitTransaction(wtxNew, reserveKey)) {
        ...
        nStatus = ZORO_COMMIT_FAILED;

        //reset all mints
        for (CZerocoinMint mint : vMintsSelected) {
            uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
            zoroTracker->SetPubcoinNotUsed(hashPubcoin);
        ...
        for (CZerocoinSpend spend : receipt.GetSpends()) {
            if (!walletdb.EraseZerocoinSpendSerialEntry(spend.GetSerial())) {
                receipt.SetStatus("Error: It cannot delete coin serial number in wallet", ZORO_ERASE_SPENDS_FAILED);
            }
        ...
        for (auto& dMint : vNewMints) {
            if (!walletdb.EraseDeterministicMint(dMint.GetPubcoinHash())) {
                receipt.SetStatus("Error: Unable to cannot delete zerocoin mint in wallet", ZORO_ERASE_NEW_MINTS_FAILED);
            }
        }
    ...
    for (CZerocoinMint mint : vMintsSelected) {
        uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
        zoroTracker->SetPubcoinUsed(hashPubcoin, txidSpend);

        CMintMeta metaCheck = zoroTracker->GetMetaFromPubcoin(hashPubcoin);
        if (!metaCheck.isUsed) {
    ...
    receipt.SetStatus("Spend Successful", ZORO_SPEND_OKAY);  // When we reach this point spending zORO was successful
...
{
    CMintMeta meta;
    if (!zoroTracker->GetMetaFromStakeHash(hashStake, meta))
...
{
    if (zoroTracker->HasPubcoin(bnValue))
        return true;
...
bool CWallet::UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CZerocoinMint mint;
    if (zoroTracker->HasPubcoinHash(hashValue)) {
        CMintMeta meta = zoroTracker->GetMetaFromPubcoin(hashValue);
        meta.nHeight = nHeight;
        meta.txid = txid;
        return zoroTracker->UpdateState(meta);
    } else {
...
bool CWallet::SetMintUnspent(const CBigNum& bnSerial)
{
    ...
    CMintMeta meta = zoroTracker->Get(hashSerial);
    zoroTracker->SetPubcoinNotUsed(meta.hashPubcoin);
    return true;
}
...
{
    // We don't even need to worry about this code.. no zORO.
    /*
    LogPrintf("Precomputer started\n");
    RenameThread("oro-precomputer");
    ...
    while (true) {
        ...
        for (std::unique_ptr <CStakeInput>& stakeInput : listInputs) {
...

            {
                ...
                CoinWitnessData* witnessData = zoroTracker->GetSpendCache(serialHash);
...
void CWallet::setZWallet(CzOROWallet* zwallet)
{
    zwalletMain = zwallet;
    zoroTracker = std::unique_ptr<CzOROTracker>(new CzOROTracker(strWalletFile));
}

CzOROWallet* CWallet::getZWallet()
{
    return zwalletMain;
}
...
void CWallet::setZOroAutoBackups(bool fEnabled)
{
...
CAmount CWallet::GetOro(const CTxOut& txout, const isminefilter& filter) const
{
    if (!MoneyRange(txout.nValue))
        throw std::runtime_error("CWallet::GetOro() : value out of range");
...
CAmount CWallet::GetOro(const CTransaction& tx, const isminefilter& filter, const bool fUnspent) const
{
    CAmount nOro = 0;
    for (int i = 0; i < tx.vout.size(); i++) {
        if (fUnspent && IsSpent(tx.GetHash(), i)) continue;
        nOro += GetOro(tx.vout[i], filter);
    }
    if (!MoneyRange(nOro))
        throw std::runtime_error("CWallet::GetOro() : value out of range");
    return nOro;
...
void CWalletTx::Init(const CWallet* pwalletIn)
{
    ...
    fDebitCached = false;
    fOroCached = false;
    fImmatureOroCached = false;
    fAvailableOroCached = false;
    fAnonymizableOroCached = false;
    fAnonymizedOroCached = false;
    fDenomUnconfOroCached = false;
    fDenomConfOroCached = false;
    fWatchDebitCached = false;
    fWatchOroCached = false;
    fImmatureWatchOroCached = false;
    fAvailableWatchOroCached = false;
    fChangeCached = false;
    fColdDebitCached = false;
    fColdOroCached = false;
    fDelegatedDebitCached = false;
    fDelegatedOroCached = false;
    fStakeDelegationVoided = false;
    nDebitCached = 0;
    nOroCached = 0;
    nImmatureOroCached = 0;
    nAvailableOroCached = 0;
    nAnonymizableOroCached = 0;
    nAnonymizedOroCached = 0;
    nDenomUnconfOroCached = 0;
    nDenomConfOroCached = 0;
    nWatchDebitCached = 0;
    nWatchOroCached = 0;
    nAvailableWatchOroCached = 0;
    nImmatureWatchOroCached = 0;
    nColdDebitCached = 0;
    nColdOroCached = 0;
    nDelegatedDebitCached = 0;
    nDelegatedOroCached = 0;
    nChangeCached = 0;
    nOrderPos = -1;
}
