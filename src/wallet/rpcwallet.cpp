...
UniValue getnewaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "\nReturns a new ORO address for receiving payments.\n"
            "If 'account' is specified (recommended), it is added to the address book \n"
            "so payments received with the address will be oroed to 'account'.\n"

            "\nArguments:\n"
            "1. \"account\"        (string, optional) The account name for the address to be linked to. if not provided, the default account \"\" is used. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created if there is no account by the given name.\n"

            "\nResult:\n"
            "\"oroaddress\"    (string) The new oro address\n"
...
UniValue getnewstakingaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "\nReturns a new ORO cold staking address for receiving delegated cold stakes.\n"
...
UniValue delegatoradd(const UniValue& params, bool fHelp)
{
    ...
    if (!address.IsValid() || address.IsStakingAddress())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid ORO address");

    CKeyID keyID;
    if (!address.GetKeyID(keyID))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Unable to get KeyID from ORO address");
...
UniValue delegatorremove(const UniValue& params, bool fHelp)
{
    ...
    if (!pwalletMain->HasAddressBook(keyID))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Unable to get ORO address from addressBook");
...
UniValue listdelegators(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "   \"address\": \"xxx\",  (string) ORO address string\n"
            "   }\n"
            "  ...\n"
            "]\n"
...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturns the current ORO address for receiving payments to this account.\n"

            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"

            "\nResult:\n"
            "\"oroaddress\"   (string) The account oro address\n"
...
UniValue getrawchangeaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new ORO address, for receiving change.\n"
...
UniValue setaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            "setaccount \"oroaddress\" \"account\"\n"
            "\nSets the account associated with the given address.\n"

            "\nArguments:\n"
            "1. \"oroaddress\"  (string, required) The oro address to be associated with an account.\n"
...
UniValue getaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "getaccount \"oroaddress\"\n"
            "\nReturns the account associated with the given address.\n"

            "\nArguments:\n"
            "1. \"oroaddress\"  (string, required) The oro address for account lookup.\n"
...
UniValue getaddressesbyaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"oroaddress\"  (string) a oro address associated with the given account\n"
            "  ,...\n"
            "]\n"
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw std::runtime_error(
            "sendtoaddress \"oroaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"oroaddress\"  (string, required) The oro address to send to.\n"
            "2. \"amount\"      (numeric, required) The amount in ORO to send. eg 0.1\n"
...
UniValue CreateColdStakeDelegation(const UniValue& params, CWalletTx& wtxNew, CReserveKey& reservekey)
{
    ...
    if (!stakeAddr.IsValid() || !stakeAddr.IsStakingAddress())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid ORO staking address");
    ...
    if (params.size() > 2 && !params[2].isNull() && !params[2].get_str().empty()) {
        ...
        if (!ownerAddr.IsValid() || ownerAddr.IsStakingAddress())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid ORO spending address");
...
UniValue delegatestake(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 6)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"stakingaddress\"      (string, required) The oro staking address to delegate.\n"
            "2. \"amount\"              (numeric, required) The amount in ORO to delegate for staking. eg 100\n"
            "3. \"owneraddress\"        (string, optional) The oro address corresponding to the key that will be able to spend the stake. \n"
...
UniValue sendtoaddressix(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw std::runtime_error(
            "sendtoaddressix \"oroaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n" +
            HelpRequiringPassphrase() + "\n"
...
UniValue listaddressgroupings(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"oroaddress\",     (string) The oro address\n"
            "      amount,                 (numeric) The amount in ORO\n"
...
UniValue signmessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw std::runtime_error(
            "signmessage \"oroaddress\" \"message\"\n"
            "\nSign a message with the private key of an address" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"oroaddress\"  (string, required) The oro address to use for the private key.\n"
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            "getreceivedbyaddress \"oroaddress\" ( minconf )\n"
            "\nReturns the total amount received by the given oroaddress in transactions with at least minconf confirmations.\n"

            "\nArguments:\n"
            "1. \"oroaddress\"  (string, required) The oro address for transactions.\n"
            "2. minconf             (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"

            "\nResult:\n"
            "amount   (numeric) The total amount in ORO received at this address.\n"
...
UniValue getreceivedbyaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in ORO received for this account.\n"
...
CAmount GetAccountBalance(CWalletDB& walletdb, const std::string& strAccount, int nMinDepth, const isminefilter& filter)
{
    ...
    if (strAccount != "*")
        nBalance += walletdb.GetAccountOroDebit(strAccount);
...
UniValue getcoldstakingbalance(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in ORO received for this account in P2CS contracts.\n"
...
UniValue movecmd(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw std::runtime_error(
            ...
            "3. amount            (numeric, required) Quantity of ORO to move between accounts.\n"
            ...
            "\nExamples:\n"
            "\nMove 0.01 ORO from the default account to the account named tabby\n" +
            HelpExampleCli("move", "\"\" \"tabby\" 0.01") +
            "\nMove 0.01 ORO from timotei to akiko with a comment\n" +
    ...
    // Oro
    CAccountingEntry oro;
    oro.nOrderPos = pwalletMain->IncOrderPosNext(&walletdb);
    oro.strAccount = strTo;
    oro.nOroDebit = nAmount;
    oro.nTime = nNow;
    oro.strOtherAccount = strFrom;
    oro.strComment = strComment;
    pwalletMain->AddAccountingEntry(oro, walletdb);
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw std::runtime_error(
            "sendfrom \"fromaccount\" \"tooroaddress\" amount ( minconf \"comment\" \"comment-to\" includeDelegated)\n"
            "\nSent an amount from an account to a oro address.\n"
            "The amount is a real and is rounded to the nearest 0.00000001." +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
            "2. \"tooroaddress\"  (string, required) The oro address to send funds to.\n"
            "3. amount                (numeric, required) The amount in ORO. (transaction fee is added on top).\n"
            "4. minconf               (numeric, optional, default=1) Only use funds with at least this many confirmations.\n"
            ...
            "\nExamples:\n"
            "\nSend 0.01 ORO from the default account to the address, must have at least 1 confirmation\n" +
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The oro address is the key, the numeric amount in ORO is the value\n"
            "      ,...\n"
            "    }\n"
    ...
    for (const std::string& name_ : keys) {
        ...
        if (!address.IsValid() || address.IsStakingAddress())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid ORO address: ")+name_);
...
UniValue addmultisigaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
            ...
            "Each key is a ORO address or hex-encoded public key.\n"
            "If 'account' is specified, assign address to that account.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keysobject\"   (string, required) A json array of oro addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) oro address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"account\"      (string, optional) An account to assign the addresses to.\n"

            "\nResult:\n"
            "\"oroaddress\"  (string) A oro address associated with the keys.\n"
...
UniValue listreceivedbyaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw std::runtime_error(
            ...
            "    \"amount\" : x.xxx,                  (numeric) The total amount in ORO received by the address\n"
...
UniValue listcoldutxos(const UniValue& params, bool fHelp)
{
    ...
    for (std::map<uint256, CWalletTx>::const_iterator it =
        ...
        if(pcoin->GetColdStakingOro() == 0 && pcoin->GetStakeDelegationOro() == 0)
            continue;
...
void AcentryToJSON(const CAccountingEntry& acentry, const std::string& strAccount, UniValue& ret)
{
    ...
    if (fAllAccounts || acentry.strAccount == strAccount) {
        ...
        entry.push_back(Pair("amount", ValueFromAmount(acentry.nOroDebit)));
...
UniValue listtransactions(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 6)
        throw std::runtime_error(
            ...
            "    \"address\":\"oroaddress\",    (string) The oro address of the transaction. Not present for \n"
            "                                                move transactions (category = move).\n"
            "    \"category\":\"send|receive|move\", (string) The transaction category. 'move' is a local (off blockchain)\n"
            "                                                transaction between accounts, and not associated with an address,\n"
            "                                                transaction id or block. 'send' and 'receive' transactions are \n"
            "                                                associated with an address, transaction id and block details\n"
            "    \"amount\": x.xxx,          (numeric) The amount in ORO. This is negative for the 'send' category, and for the\n"
            "                                         'move' category for moves outbound. It is positive for the 'receive' category,\n"
            "                                         and for the 'move' category for inbound funds.\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in ORO. This is negative and only available for the \n"
...
UniValue listaccounts(const UniValue& params, bool fHelp)
{
    ...
    for (const CAccountingEntry& entry : acentries)
        mapAccountBalances[entry.strAccount] += entry.nOroDebit;
...
UniValue listsinceblock(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw std::runtime_error(
            ...
            "    \"address\":\"oroaddress\",    (string) The oro address of the transaction. Not present for move transactions (category = move).\n"
            "    \"category\":\"send|receive\",     (string) The transaction category. 'send' has negative amounts, 'receive' has positive amounts.\n"
            "    \"amount\": x.xxx,          (numeric) The amount in ORO. This is negative for the 'send' category, and for the 'move' category for moves \n"
            "                                          outbound. It is positive for the 'receive' category, and for the 'move' category for inbound funds.\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in ORO. This is negative and only available for the 'send' category of transactions.\n"
...
UniValue gettransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"amount\" : x.xxx,        (numeric) The transaction amount in ORO\n"
            ...
            "      \"address\" : \"oroaddress\",   (string) The oro address involved in the transaction\n"
            "      \"category\" : \"send|receive\",    (string) The category, either 'send' or 'receive'\n"
            "      \"amount\" : x.xxx                  (numeric) The amount in ORO\n"
            "      \"vout\" : n,                       (numeric) the vout value\n"
            "    }\n"
            "    ,...\n"
            "  ],\n"
    ...
    CAmount nOro = wtx.GetOro(filter);
    CAmount nDebit = wtx.GetDebit(filter);
    CAmount nNet = nOro - nDebit;
...
UniValue walletpassphrase(const UniValue& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 3))
        throw std::runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending OROs\n"
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw std::runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending OROs\n" +
            HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
            "\nNow we can so something like sign\n" +
            HelpExampleCli("signmessage", "\"oroaddress\" \"test message\"") +
    ...
    StartShutdown();
    return "wallet encrypted; oro server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
UniValue lockunspent(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending OROs.\n"
...
UniValue settxfee(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. amount         (numeric, required) The transaction fee in ORO/kB rounded to the nearest 0.00000001\n"
...
UniValue getwalletinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,                      (numeric) the total ORO balance of the wallet (cold balance excluded)\n"
            "  \"delegated_balance\": xxxxx,              (numeric) the ORO balance held in P2CS (cold staking) contracts\n"
            "  \"cold_staking_balance\": xx,              (numeric) the ORO balance held in cold staking addresses\n"
            "  \"unconfirmed_balance\": xxx,              (numeric) the total unconfirmed balance of the wallet in ORO\n"
            "  \"immature_delegated_balance\": xxxxxx,    (numeric) the delegated immature balance of the wallet in ORO\n"
            "  \"immature_cold_staking_balance\": xxxxxx, (numeric) the cold-staking immature balance of the wallet in ORO\n"
            "  \"immature_balance\": xxxxxx,              (numeric) the total immature balance of the wallet in ORO\n"
            "  \"txcount\": xxxxxxx,                      (numeric) the total number of transactions in the wallet\n"
            "  \"keypoololdest\": xxxxxx,                 (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,                     (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,                   (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,                      (numeric) the transaction fee configuration, set in ORO/kB\n"
...
UniValue autocombinerewards(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || (fEnable && params.size() != 2) || params.size() > 2)
        throw std::runtime_error(
            ...
            "\nWallet will automatically monitor for any coins with value below the threshold amount, and combine them if they reside with the same ORO address\n"
...
//         throw std::runtime_error(
//             "getzerocoinbalance\n"
//             "\nReturn the wallet's total zORO balance.\n" +
//             HelpRequiringPassphrase() + "\n"

//             "\nResult:\n"
//             "amount         (numeric) Total zORO balance.\n"
...
//             "\nList all zORO mints in the wallet.\n" +
//             HelpRequiringPassphrase() + "\n"
...
//             "    \"zORO ID\": \"xxx\",   (string) Pubcoin in hex format.\n"
...
//     CWalletDB walletdb(pwalletMain->strWalletFile);
//     std::set<CMintMeta> setMints = pwalletMain->zoroTracker->ListMints(true, fMatureOnly, true);
...
//             objMint.push_back(Pair("zORO ID", m.hashPubcoin.GetHex()));     // PubCoin
...
//                     pwalletMain->zoroTracker->UpdateState(m);
//                 }
//             }
...
//     CWalletDB walletdb(pwalletMain->strWalletFile);
//     std::set<CMintMeta> setMints = pwalletMain->zoroTracker->ListMints(true, true, true);
...
//         throw std::runtime_error(
//             "listspentzerocoins\n"
//             "\nList all the spent zORO mints in the wallet.\n" +
//             HelpRequiringPassphrase() + "\n"
...
//             "\nMint the specified zORO amount\n" +
//             HelpRequiringPassphrase() + "\n"

//             "\nArguments:\n"
//             "1. amount      (numeric, required) Enter an amount of Oro to convert to zORO\n"
...
//         throw JSONRPCError(RPC_WALLET_ERROR, "zORO minting is DISABLED");
...
//         throw JSONRPCError(RPC_WALLET_ERROR, "zORO is currently disabled due to maintenance.");
...
//             "    {\n"
//             "      \"value\": amount,         (numeric) Value in ORO.\n"
//             "      \"address\": \"xxx\"         (string) ORO address or \"zerocoinmint\" for reminted change.\n"
//             "    }\n"
//             "    ,...\n"
//             "  ]\n"
//             "}\n"
...
//     bool fMintChange = params[1].get_bool();        // Mint change to zORO
//     if (fMintChange && Params().NetworkID() != CBaseChainParams::REGTEST)
//         throw JSONRPCError(RPC_WALLET_ERROR, "zORO minting is DISABLED, cannot mint change");
...
//         throw JSONRPCError(RPC_WALLET_ERROR, "zORO old spend only available in regtest for tests purposes");
//     }

//     return DoZoroSpend(nAmount, fMintChange, fMinimizeChange, vMintsSelected, address_str, ispublicspend);
// }
...
//             "\nSpend zORO mints to a ORO address.\n" +
//             HelpRequiringPassphrase() + "\n"
...
//     return DoZoroSpend(nAmount, false, true, vMintsSelected, address_str);
// }


// extern UniValue DoZoroSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool ispublicspend)
// {
...
//     CWalletDB walletdb(pwalletMain->strWalletFile);
//     CzOROTracker* zoroTracker = pwalletMain->zoroTracker.get();
...
//     CWalletDB walletdb(pwalletMain->strWalletFile);
//     CzOROTracker* zoroTracker = pwalletMain->zoroTracker.get();
//     std::set<CMintMeta> setMints = zoroTracker->ListMints(false, false, false);
...
//             "2. \"denomination\"         (integer, optional) Export a specific denomination of zORO\n"

//             "\nResult:\n"
//             "[                   (array of json object)\n"
//             "  {\n"
//             "    \"id\": \"serial hash\",  (string) the mint's zORO serial hash \n"
...
//             "    \"v\": version,   (numeric) The version of the zORO\n"
//             "    \"k\": \"privkey\"  (string) The zORO private key (V2+ zORO only)\n"
//             "  }\n"
//             "  ,...\n"
//             "]\n"
...
//     CzOROTracker* zoroTracker = pwalletMain->zoroTracker.get();
//     std::set<CMintMeta> setMints = zoroTracker->ListMints(!fIncludeSpent, false, false);
...
//             "  \"value\": amount    (numeric) The total zORO value of zerocoin mints that were added\n"
//             "}\n"
...
//         pwalletMain->zoroTracker->Add(mint, true);
//         count++;
...
//         throw std::runtime_error(
//             "reconsiderzerocoins\n"
//             "\nCheck archived zORO list to see if any mints were added to the blockchain.\n" +
//             HelpRequiringPassphrase() + "\n"
...
// UniValue setzoroseed(const UniValue& params, bool fHelp)
// {
//     if(fHelp || params.size() != 1)
//         throw std::runtime_error(
//             "setzoroseed \"seed\"\n"
//             "\nSet the wallet's deterministic zoro seed to a specific value.\n" +
//             HelpRequiringPassphrase() + "\n"

//             "\nArguments:\n"
//             "1. \"seed\"        (string, required) The deterministic zoro seed.\n"

//             "\nResult\n"
//             "\"success\" : b,  (boolean) Whether the seed was successfully set.\n"

//             "\nExamples\n" +
//             HelpExampleCli("setzoroseed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5") +
//             HelpExampleRpc("setzoroseed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5"));
...
//     CzOROWallet* zwallet = pwalletMain->getZWallet();
...
// UniValue getzoroseed(const UniValue& params, bool fHelp)
// {
//     if(fHelp || !params.empty())
//         throw std::runtime_error(
//             "getzoroseed\n"
...
//             "\nResult\n"
//             "\"seed\" : s,  (string) The deterministic zORO seed.\n"

//             "\nExamples\n" +
//             HelpExampleCli("getzoroseed", "") + HelpExampleRpc("getzoroseed", ""));
...
UniValue generatemintlist(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 2)
        throw std::runtime_error(
            "generatemintlist\n"
            "\nShow mints that are derived from the deterministic zORO seed.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments\n"
            "1. \"count\"  : n,  (numeric) Which sequential zORO to start with.\n"
            "2. \"range\"  : n,  (numeric) How many zORO to generate.\n"
    ...
    CzOROWallet* zwallet = pwalletMain->zwalletMain;
...
// UniValue dzorostate(const UniValue& params, bool fHelp) {
//     if (fHelp || params.size() != 0)
//         throw std::runtime_error(
//                 "dzorostate\n"
//                         "\nThe current state of the mintpool of the deterministic zORO wallet.\n" +
//                 HelpRequiringPassphrase() + "\n"
...
//     obj.push_back(Pair("dzoro_count", nCount));
...
void static SearchThread(CzOROWallet* zwallet, int nCountStart, int nCountEnd)
{
    ...
    try {
        ...
        for(int i = nCountStart; i < nCountEnd; i++) {
            ...
            CKey key;
            zwallet->SeedToZORO(zerocoinSeed, bnValue, bnSerial, bnRandomness, key);
...
// UniValue searchdzoro(const UniValue& params, bool fHelp)
// {
//     if(fHelp || params.size() != 3)
//         throw std::runtime_error(
//             "searchdzoro\n"
//             "\nMake an extended search for deterministically generated zORO that have not yet been recognized by the wallet.\n" +
//             HelpRequiringPassphrase() + "\n"

//             "\nArguments\n"
//             "1. \"count\"       (numeric) Which sequential zORO to start with.\n"
//             "2. \"range\"       (numeric) How many zORO to generate.\n"
//             "3. \"threads\"     (numeric) How many threads should this operation consume.\n"

//             "\nExamples\n" +
//             HelpExampleCli("searchdzoro", "1, 100, 2") + HelpExampleRpc("searchdzoro", "1, 100, 2"));
...
//     boost::thread_group* dzoroThreads = new boost::thread_group();
...
//         nPrevThreadEnd = nEnd;
//         dzoroThreads->create_thread(boost::bind(&SearchThread, zwallet, nStart, nEnd));
//     }

//     dzoroThreads->join_all();

//     zwallet->RemoveMintsFromPool(pwalletMain->zoroTracker->GetSerialHashes());
...
UniValue createautomintaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
                ...
                "\nResult\n"
                "\"address\"     (string) ORO address for auto minting\n" +
...
//             "5. \"address\"          (string, optional) ORO address to spend to. If not specified, spend to change add.\n"
...
//         throw std::runtime_error(
//             "clearspendcache\n"
//             "\nClear the pre-computed zORO spend cache, and database.\n" +
//             HelpRequiringPassphrase() + "\n"
...
//             if (fLocked) {
//                 if (zoroTracker->ClearSpendCache()) {
