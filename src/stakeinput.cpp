...
CZOroStake::CZOroStake(const libzerocoin::CoinSpend& spend)
{
...
int CZOroStake::GetChecksumHeightFromMint()
{
...
int CZOroStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZOroStake::GetChecksum()
{
    return nChecksum;
}

// The zORO block index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CZOroStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CZOroStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zORO staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
CDataStream CZOroStake::GetUniqueness()
{
    //The unique identifier for a zORO is a hash of the serial
    CDataStream ss(SER_GETHASH, 0);
    ss << hashSerial;
    return ss;
}

bool CZOroStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZOroStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zORO that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZOROOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zORO output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zORO", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZOROOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zORO output", __func__);
...
bool CZOroStake::GetTxFrom(CTransaction& tx)
{
    return false;
}

bool CZOroStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    CzOROTracker* zoroTracker = pwallet->zoroTracker.get();
    CMintMeta meta;
    if (!zoroTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zoroTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}

//!ORO Stake
bool COroStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool COroStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool COroStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount COroStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool COroStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
bool COroStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream COroStake::GetUniqueness()
{
    //The unique identifier for a ORO stake is the outpoint
...
CBlockIndex* COroStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;
