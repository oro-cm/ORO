...
const string strMessageMagic = "Oro Signed Message:\n";
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state)
{
    ...
    if (tx.IsCoinBase())
    {
    	LogPrintf("CheckTransaction--nValueOut:%d, scriptsize:%d\n", nValueOut, tx.vin[0].scriptSig.size());
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    bool rewardOk = pOroSystem->CheckBlockReward(block, nFees, state, pindex);
    if (!rewardOk) {
        return rewardOk;
    }
