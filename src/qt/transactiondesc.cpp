...
QString TransactionDesc::toHTML(CWallet* wallet, CWalletTx& wtx, TransactionRecord* rec, int unit)
{
    ...
    CAmount nNet = rec->oro + rec->debit;
    ...
    // Amount
    //
    if (wtx.IsCoinBase() && rec->oro == 0) {
        //
        // Coinbase
        //
        CAmount nUnmatured = 0;
        for (const CTxOut& txout : wtx.vout)
            nUnmatured += wallet->GetOro(txout, ISMINE_ALL);
        strHTML += "<b>" + tr("Oro") + ":</b> ";
        ...
        // Oro
        //
        strHTML += "<b>" + tr("Oro") + ":</b> " + BitcoinUnits::formatHtmlWithUnit(unit, nNet) + "<br>";
    } else {
        ...
        if (fAllFromMe) {
            ...
            for (const CTxOut& txout : wtx.vout) {
                ...
                if (toSelf)
                    strHTML += "<b>" + tr("Oro") + ":</b> " + BitcoinUnits::formatHtmlWithUnit(unit, txout.nValue) + "<br>";
            }

            if (fAllToMe) {
                // Payment to self
                CAmount nChange = wtx.GetChange();
                CAmount nValue = rec->oro - nChange;
                strHTML += "<b>" + tr("Total debit") + ":</b> " + BitcoinUnits::formatHtmlWithUnit(unit, -nValue) + "<br>";
                strHTML += "<b>" + tr("Total oro") + ":</b> " + BitcoinUnits::formatHtmlWithUnit(unit, nValue) + "<br>";
            }
            ...
            for (const CTxOut& txout : wtx.vout)
                if (wallet->IsMine(txout))
                    strHTML += "<b>" + tr("Oro") + ":</b> " + BitcoinUnits::formatHtmlWithUnit(unit, wallet->GetOro(txout, ISMINE_ALL)) + "<br>";
        }
    }
    ...
    // Message from normal oro:URI (oro:XyZ...?message=example)
