...
#include "qt/oro/privacywidget.h"
#include "qt/oro/forms/ui_privacywidget.h"
#include "qt/oro/qtutils.h"
#include "guiutil.h"
#include "qt/oro/denomgenerationdialog.h"
#include "qt/oro/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(OROGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zORO anonymizes your ORO by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zORO or convert back to ORO"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 ORO ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zORO");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zORO");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zORO");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zORO");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zORO");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zORO");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zORO");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zORO");
    ...
    // Only Convert to ORO enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzORO->setTitleClassAndText("btn-title-grey", "Total 0 zORO");
    ui->btnTotalzORO->setSubTitleClassAndText("text-subtitle", "Show denominations of zORO owned.");
    ui->btnTotalzORO->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select ORO outputs to mint into zORO.");
    ...
    connect(ui->btnTotalzORO, SIGNAL(clicked()), this, SLOT(onTotalZoroClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zORO");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of ORO to mint into zORO"));
    }else{
        btnText = tr("Convert back to ORO");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zORO to convert back into ORO"));
    }
...
void PrivacyWidget::onTotalZoroClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzORO->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzORO->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zORO is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zORO, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zORO").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zORO minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZoro(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zORO converted back to ORO"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    ui->btnTotalzORO->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
