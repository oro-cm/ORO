...
#include "qt/oro/topbar.h"
#include "askpassphrasedialog.h"
#include "qt/oro/forms/ui_topbar.h"
#include "qt/oro/lockunlock.h"
#include "qt/oro/qtutils.h"
#include "qt/oro/receivedialog.h"
#include <QPixmap>
...
TopBar::TopBar(OROGUI* _mainWindow, QWidget* parent) : PWidget(_mainWindow, parent),
                                                             ui(new Ui::TopBar)
{
    ui->setupUi(this);
    ...
    setCssProperty({ui->labelAmountTopOro, ui->labelAmountTopzOro}, "amount-small-topbar");
    setCssProperty({ui->labelAmountOro, ui->labelAmountzOro}, "amount-topbar");
    setCssProperty({ui->labelPendingOro, ui->labelPendingzOro, ui->labelImmatureOro, ui->labelImmaturezOro}, "amount-small-topbar");

    ui->labelAmountTopzOro->setVisible(false);
    ui->label_16->setVisible(false);
    ui->labelTitle2->setVisible(false);
    ui->labelAmountzOro->setVisible(false);
    ui->labelTitle5->setVisible(false);
    ui->labelPendingzOro->setVisible(false);
    ui->labelTitle6->setVisible(false);
    ui->labelImmaturezOro->setVisible(false);
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& zerocoinBalance, const CAmount& unconfirmedZerocoinBalance, const CAmount& immatureZerocoinBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance, const CAmount& delegatedBalance, const CAmount& coldStakedBalance)
{
    ...
    CAmount oroAvailableBalance = balance + delegatedBalance - nLockedBalance;

    // zORO Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalOro = GUIUtil::formatBalance(oroAvailableBalance, nDisplayUnit);
    QString totalzOro = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);
    // Top
    ui->labelAmountTopOro->setText(totalOro);
    ui->labelAmountTopzOro->setText(totalzOro);

    // Expanded
    ui->labelAmountOro->setText(totalOro);
    ui->labelAmountzOro->setText(totalzOro);

    ui->labelPendingOro->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelPendingzOro->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));

    ui->labelImmatureOro->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
    ui->labelImmaturezOro->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
