...
#include "qt/oro/send.h"
#include "qt/oro/forms/ui_send.h"
#include "qt/oro/addnewcontactdialog.h"
#include "qt/oro/qtutils.h"
#include "qt/oro/sendchangeaddressdialog.h"
#include "qt/oro/optionbutton.h"
#include "qt/oro/sendconfirmdialog.h"
#include "qt/oro/myaddressrow.h"
#include "qt/oro/guitransactionsutils.h"
...
#include "zorocontroldialog.h"

SendWidget::SendWidget(OROGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("ORO");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zORO");
    setCssProperty(ui->pushRight, "btn-check-right");

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (ORO) or private coins (zORO)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("Enter a ORO address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 ORO");
    ...
    setCssProperty(coinIcon, "coin-icon-oro");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onOROSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onOROSelected(false);});
...
void SendWidget::refreshAmounts() {
    ...
    bool isZoro = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZoro));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZoro ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendOro = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendOro ? AskPassphraseDialog::Context::Send_ORO : AskPassphraseDialog::Context::Send_zORO, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendOro) ? send(recipients) : sendZoro(recipients)) {
...
bool SendWidget::sendZoro(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zORO is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    if (walletModel->sendZoro(
        ...
        inform(tr("zORO transaction sent!"));
        ...
        if (receipt.GetStatus() == ZORO_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zORO require a security level of 100 to successfully spend.");
        } else {
        ...
        emit message("zORO transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isORO){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any ORO to select."));
        }
    }else{
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            zOroControl->exec();
            ui->btnCoinControl->setActive(!ZOroControlDialog::setSelectedMints.empty());
            zOroControl->deleteLater();
        } else {
            inform(tr("You don't have any zORO in your balance to select."));
        }
    }
}
...
void SendWidget::onOROSelected(bool _isORO){
    isORO = _isORO;
    setCssProperty(coinIcon, _isORO ? "coin-icon-oro" : "coin-icon-zoro");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress oroAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(oroAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(oroAdd.Get(), dialog->getLabel().toStdString(), "send")) {
