...
#include "qt/oro/forms/ui_addresseswidget.h"
#include "qt/oro/addresslabelrow.h"
#include "qt/oro/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(OROGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter a ORO address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress oroAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(oroAdd.Get(), label.toUtf8().constData(),
                oroAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
