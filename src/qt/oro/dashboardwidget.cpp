...
#include "qt/oro/dashboardwidget.h"
#include "qt/oro/forms/ui_dashboardwidget.h"
#include "qt/oro/sendconfirmdialog.h"
#include "qt/oro/txrow.h"
...
DashboardWidget::DashboardWidget(OROGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of ORO staked."));  // Amount of ORO and zORO staked.
    setCssSubtitleScreen(ui->labelMessage);
    // ui->labelMessage->setVisible(false);
    
    setCssProperty(ui->labelSquareOro, "square-chart-oro");
    setCssProperty(ui->labelSquarezOro, "square-chart-zoro");
    setCssProperty(ui->labelOro, "text-chart-oro");
    setCssProperty(ui->labelZoro, "text-chart-zoro");

    // ui->labelOro->setVisible(false);
    // ui->labelZoro->setVisible(false);
    ...
    ui->labelAmountZoro->setText("0 zORO");
    ui->labelAmountOro->setText("0 ORO");
    setCssProperty(ui->labelAmountOro, "text-stake-oro-disable");
    setCssProperty(ui->labelAmountZoro, "text-stake-zoro-disable");
    ui->labelAmountZoro->setVisible(false);
    ...
    ui->btnHowTo->setText(tr("How to get ORO or zORO"));
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isOro = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZORO;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair ORO, zORO
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            oro = (pair.first != 0) ? pair.first / 100000000 : 0;
            zoro = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalOro += pair.first;
            chartData->totalZoro += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesOro.append(oro);
        chartData->valueszOro.append(zoro);

        int max = std::max(oro, zoro);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("ORO");
    set1 = new QBarSet("zORO");
    ...
    set0->append(chartData->valuesOro);
    set1->append(chartData->valueszOro);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalOro > 0 || chartData->totalZoro > 0) {
        setCssProperty(ui->labelAmountOro, "text-stake-oro");
        setCssProperty(ui->labelAmountZoro, "text-stake-zoro");
    } else {
        setCssProperty(ui->labelAmountOro, "text-stake-oro-disable");
        setCssProperty(ui->labelAmountZoro, "text-stake-zoro-disable");
    }
    forceUpdateStyle({ui->labelAmountOro, ui->labelAmountZoro});
    ui->labelAmountOro->setText(GUIUtil::formatBalance(chartData->totalOro, nDisplayUnit));
    ui->labelAmountZoro->setText(GUIUtil::formatBalance(chartData->totalZoro, nDisplayUnit, true));
