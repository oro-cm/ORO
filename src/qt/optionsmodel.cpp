...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::ORO);
    ...
    if (!settings.contains("nAnonymizeOroAmount"))
        settings.setValue("nAnonymizeOroAmount", 1000);

    nAnonymizeOroAmount = settings.value("nAnonymizeOroAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeOroAmount"))
        SoftSetArg("-anonymizeoroamount", settings.value("nAnonymizeOroAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in oro.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeOroAmount:
            return QVariant(nAnonymizeOroAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeOroAmount:
            nAnonymizeOroAmount = value.toInt();
            settings.setValue("nAnonymizeOroAmount", nAnonymizeOroAmount);
            emit anonymizeOroAmountChanged(nAnonymizeOroAmount);
            break;
        case CoinControlFeatures:
