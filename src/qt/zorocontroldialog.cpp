...
#include "ui_zorocontroldialog.h"
...
std::set<std::string> ZOroControlDialog::setSelectedMints;
std::set<CMintMeta> ZOroControlDialog::setMints;

bool CZOroControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZOroControlDialog::COLUMN_DENOMINATION || column == ZOroControlDialog::COLUMN_VERSION || column == ZOroControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZOroControlDialog::ZOroControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZOroControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zORO Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZOro->setProperty("cssClass", "text-main-purple");
    ui->labelZOro_int->setProperty("cssClass", "text-main-purple");
...
ZOroControlDialog::~ZOroControlDialog()
{
    delete ui;
}

void ZOroControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZOroControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZOroControlWidgetItem* itemDenom(new CZOroControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZOroControlWidgetItem *itemMint = new CZOroControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zoroTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zoroTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zORO.";
            else if (!mint.isSeedCorrect)
                strReason = "The zORO seed used to mint this zORO is not the same as currently hold in the wallet";
            else
...
void ZOroControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZOroControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZOro_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZOroControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZOroControlDialog::GetSelectedMints()
{
...
void ZOroControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
